...
const Subcategories = () => {
    ...
    useEffect(() => {
        const getAllSubCategories = async () => {
            const response = await fetch(`http://localhost:5000/api/v1/subcategories?page=${page}&limit=${limit}&search=${search}&status=${status}&cat_id=${category}`);
    ...
    const handleDelete = (id) => {
        fetch(`http://localhost:5000/api/v1/subcategories/${id}`, {
            method: "DELETE",
            headers: {
